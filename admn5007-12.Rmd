...
For marketing, this often means testing on:
...
An example for when you would use a t-test to evaluate an A/B test would be to measure, for instance, time as the dependent variable.

<b>Loigstic Regression</b>: Allows us to compare two groups with binary (categorical) dependent variable. 

An example for when you would use logisitc regression to evaluate an A/B test, might be for an action (did the user complete the action or not).
...
<img src="docs/test2.png" alt="LinkedIn Article with Like button moved up above the fold" stle="float:right;">
...
## Figure out Sample Size for Each Analysis
